...
// PostNoAuth http post to remote webserver without authentication
func (config WebConfig) PostNoAuth(url string, postdata string, headers Headers) ([]byte, error) {
	var (
		err    error
		client http.Client
		req    *http.Request
	...
	// setup request
	if req, err = http.NewRequest(http.MethodPost, url, bytes.NewBuffer([]byte(postdata))); err != nil {
		return output, err
	}
